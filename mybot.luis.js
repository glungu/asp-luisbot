...
var connector = new builder.ChatConnector({
    appId: "4440ed43-8cca-4e7f-aa67-ce1b2ba33904",
    appPassword: "mGUAZdjlqqtKGH48223;{{]"
});
...
bot.dialog('ShowIntent', [
    function (session, args, next) {
        ...
        var channel = session.userData.channel;
...
bot.dialog('MissedCalls', [
    function (session, args, next) {
        // Get info from LUIS.
        session.userData.intent = args.intent;
        session.userData.channel = builder.EntityRecognizer.findEntity(args.intent.entities, 'Notification.Channel');
        session.beginDialog("ShowIntent");
    }
]).triggerAction({ 
    matches: "Notification.Start.MissedCalls"
});

bot.dialog('ZeroBalance', [
    function (session, args, next) {
        // Get info from LUIS.
        session.userData.intent = args.intent;
        session.userData.channel = builder.EntityRecognizer.findEntity(args.intent.entities, 'Notification.Channel');
